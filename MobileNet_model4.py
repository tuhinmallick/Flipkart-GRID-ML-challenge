...
TEST_PATH = 'test.csv'
# changed image size from 160-120 to 240-180 and alpha from 1.4 to 1.6 and batch size from 32 to 16
ALPHA = 1.6
...
TRAIN_CSV = "train_4.csv"
VALIDATION_CSV = "val_4.csv"
...
def create_train_val_csv(seed = 42, train_val_split = 0.2):
    ...
    df_train.to_csv('train_4.csv', index = True, header = False)
    df_val.to_csv('val_4.csv', index = True, header = False)
...
#            filename contains test_images/ which is not there in test.csv
    ...
    test.to_csv('predictions_MobileNet_model4.csv', encoding='utf-8', index=True)
...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_images = np.zeros((len(batch_paths), 240, 180, 3), dtype=np.float32)
...
def create_model(trainable=True):
    image_input = Input(shape=(240, 180, 3))
...
def main():
    ...
    checkpoint = ModelCheckpoint("./Mobilenet_checkpoints/model-4-{epoch:02d}-{val_iou:.2f}-.h5", monitor="val_iou", verbose=1, save_best_only=True,
