...
def create_train_val_csv(seed = 42, train_val_split = 0.2):
    ...
    df_train.to_csv('train_2.csv', index = True, header = False)
    df_val.to_csv('val_2.csv', index = True, header = False)
...
#            filename contains test_images/ which is not there in test.csv
    ...
    test.to_csv('predictions_MobileNet_model2.csv', encoding='utf-8', index=True)
...
def main():
    create_train_val_csv(seed = 84, train_val_split = 0.2)
    ...
    checkpoint = ModelCheckpoint("./Mobilenet_checkpoints/model-2-{epoch:02d}-{val_iou:.2f}-.h5", monitor="val_iou", verbose=1, save_best_only=True,
